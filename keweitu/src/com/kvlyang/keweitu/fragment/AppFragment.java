...
import com.kvlyang.keweitu.listview.item.AppHolder;
...
public class AppFragment extends BaseFragment {
	...
	protected LoadedDataAndView initDataFromCaches() {
		...
		String result = CacheDataLoad.loadDataFromFile("app", 0);
		if( result == null){
	...
	//	SystemClock.sleep(4000);
		...
		if (!CacheDataLoad.isCachaOutTime("app", 0)){
		...
		try {
			// 解析json网络数据
			result =  HttpProtocal.loadHttpData("app",0);
			Gson gson = new Gson();
			List<AppInfoBean> bean = gson.fromJson(result,new TypeToken<List<AppInfoBean>>(){}.getType());
			// 异常处理，显示网络异常界面或缓存
			if (bean == null) {
		...
		//保存到缓存
		CacheDataLoad.writeDataToFile("app", 0, result);
	...
	protected View initSuccessView(Object data) {
		...
		AppAdapter homeAdapter = new AppAdapter(homeDatas.mDatas);
	...
	class AppAdapter extends BaseAdapterKwt<AppInfoBean> {

		public AppAdapter(List<AppInfoBean> mDatas) {
			super(mDatas);
		}
		...
		//	SystemClock.sleep(3000);
			String result = HttpProtocal.loadHttpData("app",mListData.size());
			Gson gson = new Gson();
